...
void MainWindow::readSerialData() {
    if (arduino.getserial()->isReadable()) {
            ...
            if (isValid) {
                ...
                if (query.exec()) {
                    if (query.next()) {
                        ...
                        ui->label_nom_four->setText(nom);  // Afficher le nom dans le QLabel
        ...
        // Afficher les données reçues dans le QLabel test_omar
...
void MainWindow::sendNameToLCD(const QString &nom) {
    // Convert the name to UTF-8 and send it via Arduino
    arduino.write_to_arduino(nom.toUtf8());
    arduino.write_to_arduino("\n");  // Optional: end the transmission with a newline
}
